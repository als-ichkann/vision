...
# coding=utf-8
"""
基于PyBullet仿真的视觉伺服驱动系统
整合PBVS和IBVS功能，支持仿真和真实机械臂切换
"""
...
# 根据仿真模式选择导入
SIMULATION_MODE = True  # 设置为True使用仿真，False使用真实机械臂

if SIMULATION_MODE:
    # 导入PyBullet仿真机器人控制模块
    from pybullet_robot_adapter import Auboi5Robot, RobotErrorType, logger_init
    print("使用PyBullet仿真模式")
else:
    # 导入真实机器人控制模块
...
class RobotVisualServoing:
    """机器人视觉伺服系统（支持仿真和真实机械臂）"""
    
    def __init__(self, robot_ip='localhost', robot_port=8899, 
                 model_path='yolo_train/weights/best.pt', simulation_mode=None):
        """初始化视觉伺服系统"""
        
        # 如果传入了simulation_mode参数，使用该参数
        ...
        # 机器人控制器
        if SIMULATION_MODE:
            self.robot = Auboi5Robot(gui=True)  # PyBullet仿真
        else:
            self.robot = Auboi5Robot()  # 真实机械臂
        ...
        print(f"视觉伺服系统初始化完成 - {'仿真模式' if self.simulation_mode else '真实机械臂模式'}")
        
    def setup_camera(self):
        """设置相机（RealSense或PyBullet仿真相机）"""
        if self.simulation_mode:
            # PyBullet仿真模式下，相机由机械臂适配器提供
            print("使用PyBullet仿真相机")
            self.pipeline = None
            self.config = None
            self.depth_scale = 0.001  # PyBullet仿真深度比例
            self.align = None
        else:
            # 真实RealSense相机设置
            try:
    ...
    def connect_robot(self):
        ...
        try:
            ...
            print(f"机器人连接成功 - {'仿真模式' if self.simulation_mode else '真实机械臂'}")
            return True
        except Exception as e:
            print(f"机器人连接失败: {e}")
            return False
    ...
    def get_frames(self):
        """获取RGB-D帧（RealSense或PyBullet仿真）"""
        if self.simulation_mode:
            # PyBullet仿真模式
            try:
                color_image, depth_image = self.robot.get_camera_image()
                # 在仿真模式下需要推进仿真
                self.robot.step_simulation()
                return color_image, depth_image
            except Exception as e:
                print(f"仿真相机获取图像失败: {e}")
                return None, None
        else:
            # 真实RealSense相机
            try:
    ...
    def run_pbvs(self):
        ...
        try:
            while True:
                ...
                window_name = f'Robot PBVS - {"仿真" if self.simulation_mode else "真实"}'
    ...
    def run_ibvs(self):
        ...
        try:
            while True:
                ...
                window_name = f'Robot IBVS - {"仿真" if self.simulation_mode else "真实"}'
                cv2.imshow(window_name, color_image)
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'):
                    break
    ...
    def visualize_pbvs(self, image, target):
        ...
        # 状态信息
        mode_text = "仿真" if self.simulation_mode else "真实"
        status = f"PBVS Mode ({mode_text})" + (" - EMERGENCY STOP" if self.emergency_stop else "")
    ...
    def visualize_ibvs(self, image, target):
        ...
        # 状态信息
        mode_text = "仿真" if self.simulation_mode else "真实"
        status = f"IBVS Mode ({mode_text})" + (" - EMERGENCY STOP" if self.emergency_stop else "")
        cv2.putText(image, status, (10, image.shape[0] - 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255) if self.emergency_stop else (255, 255, 255), 2)
    
    def cleanup(self):
        """清理资源"""
        if not self.simulation_mode and self.pipeline:
            try:
                self.pipeline.stop()
            except:
                pass
...
def main():
    ...
    print()
    print("模式选择:")
    print("S. PyBullet仿真模式")
    print("R. 真实机械臂模式")
    print()
    
    mode_choice = input("请选择运行模式 (S/R): ").strip().upper()
    simulation_mode = mode_choice == 'S'
    ...
    control_choice = input("请选择控制方法 (1-2): ").strip()
    
    # 获取机器人连接参数（仿真模式下这些参数会被忽略）
    if simulation_mode:
        robot_ip = "localhost"
        robot_port = 8899
        print(f"仿真模式：使用默认参数 {robot_ip}:{robot_port}")
    else:
